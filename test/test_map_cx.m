...
sum_mse_ml = (~data(:,3))' * (exp(-((data(:, 1) - mean_x1).^2./(2*var_x1)) ...
						-((data(:, 2) - mean_x2).^2./(2*var_x2)))*w_map + w0_map - t_data).^2;

land_num = sum(data(:,3)==0);
mse_map_cx = sqrt(sum_mse_ml/land_num);

save -append -mat "~/Spring_2016/ML/2016_ML_HW1_v4/test/test_result.mat" mse_map_cx;
